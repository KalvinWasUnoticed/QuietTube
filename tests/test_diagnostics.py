...
class DiagnosticTests(unittest.TestCase):
 def test_writer_fixed_schema_no_payload_dump(self):
  ...
  for forbidden in ['localizedDescription','NSLog(', 'NSURLConnection','NSURLSession','setBool:', 'QTSet(', 'description]']:
 ...
 def test_manual_state_is_not_a_preference(self):
  s=(R/'Sources/QTDiagnosticLog.m').read_text()
  self.assertNotIn('NSUserDefaults',s)
 ...
 def test_lifecycle_and_ui_main_thread_export(self):
  ...
  for name in ['startDiagnostics','stopDiagnostics','exportDiagnostics','clearDiagnostics']:self.assertIn(name,s)
