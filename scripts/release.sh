...
ipa='artifacts/QuietTube-0.10-21.38.2.ipa'
[[ -s "$ipa" ]] || { echo 'IPA missing or empty' >&2; exit 1; }
tag="quiettube-0.10-${GITHUB_RUN_ID}-${GITHUB_RUN_ATTEMPT}"
server="${GITHUB_SERVER_URL:-https://github.com}"
page="${server}/${GITHUB_REPOSITORY}/releases/tag/${tag}"
download="${server}/${GITHUB_REPOSITORY}/releases/download/${tag}/QuietTube-0.10-21.38.2.ipa"
notes="${RUNNER_TEMP:-.}/quiettube-release-notes.md"
cat > "$notes" <<NOTES
# QuietTube 0.10 — personal LiveContainer test build
...
gh release create "$tag" "$ipa" --repo "$GITHUB_REPOSITORY" --target "$GITHUB_SHA" \
  --prerelease --title "QuietTube 0.10 — build ${GITHUB_RUN_ID}.${GITHUB_RUN_ATTEMPT}" --notes-file "$notes"
cat >> "$GITHUB_STEP_SUMMARY" <<SUMMARY
# ✅ [DOWNLOAD IPA — QuietTube 0.10](${download})
